...
export default function SharedLayout({ children, role }: SharedLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <aside
        className={`hidden md:flex flex-col bg-white border-r border-gray-200 transition-all duration-300 ease-in-out z-20 ${isExpanded ? 'w-64' : 'w-20'
          }`}
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-gray-100">
          ...
          <button
            ...
            className="p-2 rounded-lg text-gray-400 hover:text-blue-600 transition-colors mx-auto"
          >
        ...
        <nav className="flex-1 py-6 flex flex-col gap-2 px-3">
          {links.map((link, index) => (
            <button
              ...
              className="flex items-center px-3 py-3 rounded-xl transition-colors text-gray-500 hover:text-blue-600 group"
      ...
      <header className="md:hidden bg-white h-16 border-b border-gray-200 flex items-center justify-center sticky top-0 z-10">
        <HeartPulse className="w-6 h-6 text-blue-600 mr-2" />
        <span className="font-bold text-lg text-gray-900">HealthSync</span>
      </header>

      <main className="flex-1 flex flex-col overflow-x-hidden min-h-[calc(100vh-8rem)] md:min-h-screen pb-20 md:pb-0">
        {children}
      </main>
